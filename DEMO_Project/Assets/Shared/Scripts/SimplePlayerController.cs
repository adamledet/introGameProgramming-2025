...
namespace Exercises
{
    ...
    public class SimplePlayerController : SimplePlayerControllerBase
    {
        ...
        private void OnEnable()
        {
            ...
            inputManager.onJump.AddListener(() => m_StartJumping = true);
        }
        ...
        void Update()
        {
            if (enabled)
            {
                ...
                var rawInput = inputManager.rawInput;
                ...
                if (true)//!m_IsJumping)
                {
                    m_IsSprinting = inputManager.Sprint.Value > 0.5f;
        ...
        void ApplyMotion()
        {
            ...
            {
                if (!zMovement)
                    m_CurrentVelocityXZ = new Vector3(m_CurrentVelocityXZ.x,0,0);
                m_Controller.Move((m_CurrentVelocityY * UpDirection + m_CurrentVelocityXZ) * Time.deltaTime);
                animator.SetFloat("Blend", m_CurrentVelocityXZ.magnitude * (IsSprinting ? 2 : 1));
            }
            else
            {
        ...
        float GetDistanceFromGround(Vector3 pos, Vector3 up, float max)
        {
            ...
            if (Physics.Raycast(offset+pos + up * kExtraHeight, -up, out var hit, 
                    max + kExtraHeight, GroundLayers, QueryTriggerInteraction.Ignore))
                list.Add(hit.distance - kExtraHeight);
            if (Physics.Raycast(-offset+pos + up * kExtraHeight, -up, out hit,
                    max + kExtraHeight, GroundLayers, QueryTriggerInteraction.Ignore))
                list.Add(hit.distance - kExtraHeight);
            if (Physics.Raycast(pos + up * kExtraHeight, -up, out hit,
                    max + kExtraHeight, GroundLayers, QueryTriggerInteraction.Ignore))
                list.Add(hit.distance - kExtraHeight);
            list.Add(max + 1);
            return list.Min();
